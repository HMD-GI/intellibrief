<template>
  <div class="brief-table">
    ...
    <div v-for="item in items" :key="item.id" class="table-row">
      <div>{{ item.date }}</div>
      <div>{{ item.title }}</div>
      <div>{{ item.topic || "综合" }}</div>
      <div>{{ item.type || "daily" }}</div>
      <div class="row-actions">
        <button type="button" class="secondary-btn" @click="$emit('view', item)">查看</button>
        <button type="button" class="danger-btn" @click="$emit('remove', item)">删除</button>
      </div>
    </div>
  </div>
</template>
